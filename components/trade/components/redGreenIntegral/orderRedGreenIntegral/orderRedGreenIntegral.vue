<template>
	<view :style="'padding:'+datas.content.padding+'px 0;'">
		<view class="custom-order">
			<view class="type2" v-if="datas.content.css_type==2">
				<view hover-class="no-hover" url='/red_green_integral/pages/order/orderList/orderList' open-type='navigate'>
					<view class="order-title" v-for="(it,it_index) in list" v-if="it_index<1">
						<view class="image">
							<image :src="it.pic"></image>
						</view>
						<view class="infor">
							{{datas.content.li_title}}
						</view>
					</view>
				</view>
			</view>
			<view class="type1" v-if="datas.content.css_type==1">
				<view class="title">
					<p class="name">{{datas.content.li_title}}</p>
					<view hover-class="no-hover"
						@click="$common.diyLinkJump('/red_green_integral/web/index.php?m=order&a=order_list')">
						<p class="more">全部订单</p>
					</view>
				</view>
				<view class="order-list">
					<view class="li" v-if="datas.content.order_style==undefined||datas.content.order_style==1" v-for="(itm,itm_index) in datas.content.dataset[0].icon_list">
						<view class="nav-num" hover-class="no-hover"
							@click="$common.diyLinkJump('/red_green_integral/web/index.php?m=order&a=order_list&status='+itm_index)">
							<view class="image">
								<image :src="itm.icon" alt=""> </image>
							</view>
							<p>{{itm.name}}</p>
							<block v-if='itm.num>0'>
								<view class='reddot' v-if="itm.num>99">99+</view>
								<view class='reddot' v-else>{{itm.num}}</view>
							</block>
						</view>
					</view>
					
					<view class="li" v-if="datas.content.order_style==2"  v-for="(itm,itm_index) in datas.content.dataset[0].icon_list_two">
						<view class="nav-num" @click="$common.diyLinkJump('/red_green_integral/web/index.php?m=order&a=order_list&status='+itm_index)">
							<view class="image">
								<image :src="itm.icon" alt=""> </image>
							</view>
							<p>{{itm.name}}</p>
							<block v-if='itm.num>0'>
								<view class="reddot" style="width: 24px;" v-if="itm.num>99">99+</view>
								<view class="reddot" v-else>{{itm.num}}</view>
							</block>
						</view>
					</view>
				</view>
			</view>
		</view>
	</view>
</template>

<script>
	export default {
		name: "orderRedGreenIntegral",
		props: {
			datas: {
				type: Object,
				default: {}
			},
		},
		data() {
			return {
				list: []
			};
		},
		created() {

			var that = this
			that.get_order_num();
			var list = that.datas.content.dataset
			var reg = new RegExp(/^[hH][tT][tT][pP]([sS]?):\/\/(\S+\.)+\S{2,}$/);
			// console.log('红绿积分订单1',list)
			// list.forEach(function(item, index) {
			// 	if (!reg.test(item.icon)) {
			// 		if (item.icon.indexOf('..') != -1) {
			// 			item.icon = that.vuex_apiUrl + item.icon.replace(/../, "")
			// 		} else {
			// 			item.icon = that.vuex_apiUrl + '/resources/' + item.icon
			// 		}
			// 	}
			// })
			// console.log('红绿积分订单',list)
			// that.list = list
		},
		methods: {
			
				/**
				 * 获取订单数量
				 */
				get_order_num: function() {
					const that = this;
					var params = {
					};
					that.$common.requestData({
						url: '/red_green_integral/web/index.php?m=order&a=get_order_num',
						data: params,
						method: "POST", 
						needToken: true
					}).then(res => {
						if(res.errcode==0){
							if(res.data.pend_pay > 0){//待付款
								that.datas.content.dataset[0].icon_list[1].num = res.data.pend_pay;
							}
							if(res.data.pend_send > 0){//代发货
								that.datas.content.dataset[0].icon_list[2].num = res.data.pend_send;
							}
							if(res.data.pend_receive > 0){//待收货
								that.datas.content.dataset[0].icon_list[3].num = res.data.pend_receive;
							}
							if(that.datas.content.order_style==2){
								if(res.data.pend_pay > 0){//待付款
									that.datas.content.dataset[0].icon_list_two[1].num = res.data.pend_pay;
								}
								if(res.data.pend_send > 0){//代发货
									that.datas.content.dataset[0].icon_list_two[2].num = res.data.pend_send;
								}
								if(res.data.pend_receive > 0){//待收货
									that.datas.content.dataset[0].icon_list_two[3].num = res.data.pend_receive;
								}
							}
						}
					})
			
				},
		}
	}
</script>

<style>
	.no-hover {
		background-color: transparent
	}

	.custom-order {
		padding: 0 10px;

	}

	.custom-order .order-title {
		display: block;
		text-align: center
	}

	.custom-order .type2 {
		padding: 0px 10px;
		border-radius: 8px;
		background-color: #FFF;
	}

	.reddot {
		width: 30rpx;
		height: 30rpx;
		border-radius: 50rpx;
		background: red;
		float: right;
		margin-right: 10px;
		color: white;
		position: absolute;
		left: 96rpx;
		top: -4rpx;
		font-size: 18rpx
	}

	.custom-order .order-title {
		padding: 13.5px 15px 13.5px 0;
		overflow: hidden;
		background-image: url(data:image/png;base64,);
		background-size: auto 14px;
		background-repeat: no-repeat;
		background-position: right center;
	}

	.custom-order .order-title .image {
		width: 28px;
		height: 28px;
		float: left;
		margin-right: 10px;
	}

	.custom-order .order-title .image image {
		display: block;
		width: 100%;
		height: 100%;
	}

	.custom-order .order-title .infor {
		width: calc(100% - 38px);
		float: left;
		line-height: 28px;
		color: #333;
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
		text-align: left;
	}

	.custom-order .type1 {
		background-color: #FFF;
		border-radius: 8px;
		padding: 0 15px;
	}

	.custom-order .type1 .nav-num {
		text-align: center;
		position: relative
	}

	.custom-order .type1 .title {
		display: -webkit-box;
		display: -webkit-flex;
		display: flex;
		padding: 10px 0;
		font-size: 15px;
		color: #333;
		line-height: 25px;
	}

	.custom-order .type1 .title p {
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
		margin: 0;
	}

	.custom-order .type1 .title .name {
		-webkit-box-flex: auto;
		-webkit-flex: auto;
		flex: auto;
		width: 50%;
	}

	.custom-order .type1 .title .more {
		-webkit-box-flex: none;
		-webkit-flex: none;
		flex: none;
		margin-left: 10px;
		background-image: url(data:image/png;base64,);
		background-size: auto 14px;
		background-repeat: no-repeat;
		background-position: right center;
		padding-right: 15px;
		font-size: 13px;
		color: #999;
	}

	.custom-order .type1 .order-list {
		display: -webkit-box;
		display: -webkit-flex;
		display: flex;
		text-align: center;
		padding: 10px 0 15px;
	}

	.custom-order .type1 .order-list .li {
		-webkit-box-flex: auto;
		-webkit-flex: auto;
		flex: auto;
		width: 20%;
	}

	.custom-order .type1 .order-list .image {
		width: 30px;
		height: 30px;
		margin: 0 auto 5px;
	}

	.custom-order .type1 .order-list .image image {
		display: block;
		width: 100%;
		height: 100%;
	}

	.custom-order .type1 .order-list p {
		font-size: 13px;
		color: #333333;
		line-height: 24px;
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
		display: block
	}
</style>
