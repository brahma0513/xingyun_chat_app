<template>
	<view :style="'padding:'+datas.content.padding+'px 0px '+datas.content.padding_bottom+'px;'" >
		...
		<!--弹窗start-->
		<u-modal :show="notifyShow" :showCancelButton="true" :title="$t('public.modal-title')" 
		@confirm="handlePopConfirm" @cancel="handleCancel">
			<view class="slot-content" style="text-align: center;color: #999999;">
				{{$t('newShopLeagueStore.location-tips')}}
			</view>
		</u-modal>
		<!--弹窗end-->
		<view class="page-tab-main" v-if="datas.content.css_type == 1">
			<view class="page-tab-list flex-def flex-zAround flex-cCenter">
				<view :class="'list flex-def flex-cCenter flex-zCenter '+(datas.content.show_industry_main?'avice skin-color-'+theme:'')" @click="league_toggleIndustry">
					{{datas.content.industry_name}}<span class="arrow"><image class="arrow_img" :src="http_host+'/fkshop_league/web/static/images/xia_jiantou.png'"></span>
				</view>
				<view :class="'list '+(datas.content.tab_sales?'skin-color-'+theme:'')" @click="league_salesFun">
					{{$t('newShopLeagueStore.highest-sales')}}
				</view>
				<view :class="'list '+(datas.content.tab_distance?'skin-color-'+theme:'')" @click="league_distanceFun">
					{{$t('newShopLeagueStore.nearest')}}
				</view>
		...
		<view class="mescroll-main" :style="'padding: 0px '+datas.content.padding_horizontal+'px;'">
			<view class="mescroll-list">
				<view class="list flex-def" v-for="(itm,idx) in datas.content.dataset_default" v-if="datas.content.dataset.length == 0 || datas.content.add_type == 2 || datas.content.add_type == 3" @click="goUrl('/fkshop_league/web/index.php?m=store&a=store_detail&id='+itm.id)">
			  ...
			  <image v-if="itm.has_union_card" style="width:16px;height:16px" :src="http_host+'/fkshop_league/web/static/images/icon_vip@3x.png'"></image>
			</view>
						<view class="center flex-def flex-cCenter" :class="datas.content.draw_proportion&&itm.show_percentage?'margin-bottom-0':'margin-bottom-22'">
							<view class="score">
								<span class="score-bg score-up" :style="'width:'+(itm.score/5)*100+'%;background-image: url('+http_host+'/fkshop_league/web/static/images/score_2.png)'"></span>
								<span class="score-bg score-down" :style="'background-image: url('+http_host+'/fkshop_league/web/static/images/score_2.png)'"></span>
							</view>
							<view class="num">￥{{itm.price}}/{{$t('newShopLeagueStore.unit-people')}}</view>
							<view class="num">{{$t('cssIntegralShop.sale')}}{{itm.sold}}</view>
						</view>
						<view v-if="datas.content.draw_proportion&&itm.show_percentage" class="flex-def flex-cCenter new-trade100361-draw-proportion-box">
							...
							<image class="new-trade100361-draw-proportion-bg" :src="http_host+'/fkshop_league/web/static/images/bg_new_trade100361_proportion.png'"></image>
			...
			<view class="promotion-shop-box" v-if="itm.main_push_product.length>0">
				<view class="promotion-shop flex-def flex-cEnd" v-for="(items,indexs) in itm.main_push_product" @click="goUrl('/fkshop_league/web/index.php?m=product&a=product_detail&store_id='+itm.id+'&id='+itm.pro_id)" :data-id="itm.id" :data-pid="items.pro_id">
					<view class="icon flex-def flex-zCenter flex-cCenter">{{$t('newShopLeagueStore.discounts')}}</view>
				...
				<view class="list flex-def" v-for="(itm,index1) in datas.content.dataset" v-if="datas.content.dataset.length > 0 && datas.content.add_type == 1" @click="goUrl('/fkshop_league/web/index.php?m=store&a=store_detail&id='+itm.id)">
					<view class="img-box flex-def flex-zCenter flex-cCenter"><image :src="itm.pic[0]" style="width:78px;height:78px" mode="aspectFill"/></view>
					<view class="text-box flex-def flex-zTopBottom flex-zBetween">
			<view class="flex-def flex-cCenter flex-zBetween">
			  <view class="name margin-bottom-0">{{itm.name}}</view>
			...
			<view class="promotion-shop-box" v-if="itm.main_push_product.length>0">
				<view class="promotion-shop flex-def flex-cEnd" v-for="(items,indexs) in itm.main_push_product" @click="goUrl('/fkshop_league/web/index.php?m=product&a=product_detail&store_id='+items.id+'&id='+items.pro_id)" :data-id="itm.id" :data-pid="items.pro_id">
					<view class="icon flex-def flex-zCenter flex-cCenter">{{$t('newShopLeagueStore.discounts')}}</view>
					<view class="price-box flex-def flex-cEnd">
						<span class="discount">￥{{items.price}}</span>
						<span class="price">￥{{items.package_price}}</span>
				...
				<view v-if="datas.content.isLoading">
					<view class="shop-league-not" v-if="((datas.content.dataset.length == 0 || datas.content.add_type == 2 || datas.content.add_type == 3) && datas.content.dataset_default.length == 0) || (datas.content.dataset.length == 0 && datas.content.add_type == 1)">
						<image :src="http_host+'/fkshop_league/web/view/images/jianshezhong.png'"/>
						<view class="shop-league-text">{{$t('newShopLeagueStore.store-tips')}}</view>
					</view>  
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			get_location: function (isLocate) {
				...
				if(isLocate==true){
					uni.showLoading({
						title: that.$t('offlineShoppingStoreList.positioning')
					})
					uni.getLocation({
						type: 'gcj02',
						...
						fail:function(res){
						  ...
						  uni.getLocation({
								...
								fail:function(res){
								  console.log('get_location_fail',res)
								  uni.showToast({
									title: that.$t('offlineShoppingStoreList.positio-err'),
									icon: 'none'
								  })
								  uni.hideLoading()
								  that.shop_league_get_list();
			...
			league_haopingFun: function () {/*好评筛选切换*/
			      var that = this
			      that.datas.content.tab_sales = false;
				  that.datas.content.tab_distance = false;
