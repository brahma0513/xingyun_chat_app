				...
				<block v-for="(item,index) in datasetlist">
					<view class="list" :style="'margin-left: '+(index==0?datas.content.padding_left:0)+'px;'">
						<navigator hover-class="no-hover" @click="goUrl(item)"
							:url="item.sel_link_type==2||item.sel_link_type==3? '':item.mini_url" open-type="navigate">
		...
		methods: {
			...
			fkshop_league_getdata: function() {
				...
				if (that.fkshop_league_obj.cache_position) { //有缓存，使用缓存的定位加载数据
					...
					that.fkshop_league_obj.latitude = that.fkshop_league_obj.cache_position.lat;
					that.fkshop_league_obj.longitude = that.fkshop_league_obj.cache_position.lng;
					that.fkshop_league_obj.city = that.fkshop_league_obj.cache_position.city;
					that.fkshop_league_obj.province = that.fkshop_league_obj.cache_position.province;
					setTimeout(function() {
				...
				console.log(that.fkshop_league_obj)
			},
			...
			fkshop_league_get_list: function() {
				...
				if(uni.getStorageSync('current_city')){
					current_city = JSON.parse(uni.getStorageSync('current_city'))
				}
				if (!current_city) {
					//lbs定位未完成，500ms后重试
					setTimeout(function() {
				...
				that.$common.requestData({
					url: '/fkshop_league/web/index.php?m=advertis_image&a=get_image',
					data: {
						city_code: city_code
					},
					method: 'POST',
