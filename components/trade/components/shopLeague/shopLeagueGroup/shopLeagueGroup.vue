						...
						<view
							...
							<span class="arrow">
								<image class="arrow_img"
									:src="http_host+'/shop_league/web/static/images/xia_jiantou.png'" />
							</span>
						</view>
					</view>
					...
					<view class="mescroll-main" v-if="datasetitms&&datasetitms.display_type==1">
						<view class="mescroll-list">
							<view class="list flex-def" v-for="(itm,indexs) in datasetitms.store_dataset_default"
								...
								@click="goUrl"
								:data-url="'/shop_league/pages/store/storeDetail/storeDetail?id='+itm.id" :data-id="itm.id">
								...
								<view class="text-box flex-def flex-zTopBottom flex-zBetween">
									<view class="flex-def flex-cCenter flex-zBetween">
										...
										<image v-if="itm.has_union_card" style="width:16px;height:16px"
											:src="http_host+'/shop_league/web/static/images/icon_vip@3x.png'"></image>
									</view>
									<view class="center flex-def flex-cCenter margin-bottom-22">
										<view class="score">
											<span class="score-bg score-up"
												:style="'width:'+(itm.score/5)*100+'%;background-image: url('+http_host+'/shop_league/web/static/images/score_2.png)'"></span>
											<span class="score-bg score-down"
												:style="'background-image: url('+http_host+'/shop_league/web/static/images/score_2.png)'"></span>
										</view>
							...
							<view v-if="datas.content.isLoading">
								<view class="shop-league-not"
									...
									<image :src="http_host+'/shop_league/web/view/images/jianshezhong.png'" style="margin: 0 auto;"/>
					...
					<view class="mescroll-main" v-if="datasetitms&&datasetitms.display_type==2">
						<view class="mescroll-shop-list">
								...
								<view class="text-box">
									...
									<view class="store flex-def flex-cCenter flex-zBetween">
										<view class="store-name flex-def flex-cCenter">
											<image :src="http_host+'/shop_league/web/static/images/shop_icon.png'"
												alt="" />
							...
							<view
								...
								<image :src="http_host+'/shop_league/web/static/images/nobenefits@3x.png'" alt=""
	...
	export default {
		...
		created() {
			...
			uni.$on('scrollLowerComp', () => {
				...
				var store_list = that.datasetitms.dataset;
				if(that.can_load==true&&that.last_key<that.total_count){
					...
					for(var i=0; i<=that.page_size;i++){
						if(i<that.page_size&&that.last_key<that.total_count){
							store_list.push(that.all_list[that.last_key])
							that.last_key = parseInt(that.last_key)+1;
						}
						if(i+1==that.page_size){
							that.datasetitms.dataset = store_list
							that.can_load = true
						}
		...
		methods: {
			...
			get_location: function() {
				...
				uni.getLocation({
					...
					success: function(res) {
						console.log('get_location_success', res)
			...
			shop_league_getdata: function() {
				...
				console.log("进入获取定位33");
				that.get_location()
				console.log("进入获取定位44");
				...
				if (itm.display_type == 1) {
					...
					that.$common.requestData({
						...
						if (res.errcode == 0) {
							...
							that.all_list = itm.dataset
							that.total_count = that.all_list.length;
							//加载第一页数据
							that.can_load = false;
								console.log("全部数据")
								console.log(that.all_list)
								console.log('page_size',that.page_size)
							for(var i=0; i<that.page_size;i++){
								console.log("循环",that.last_key)
								if(i<that.page_size&&that.last_key<that.total_count){
									store_list.push(that.all_list[that.last_key])
									that.last_key = parseInt(that.last_key)+1;
								}
					...
					if(itm.commodity_type == 1||itm.commodity_type == 101){
					  var url = "/shop_league/web/index.php?m=product&a=get_all_product_list&xdebug=xdebug";
					  if(itm.commodity_type == 101){
						  search.is_plat_product=true
					  }else{
						  search.is_plat_product=false
					  }
					}else if(itm.commodity_type == 2){
					  var url ="/shop_league/web/index.php?m=user_collage_product&a=collage_product_list_api";
					}else if(itm.commodity_type == 3){
					  var url ="/shop_league/web/index.php?m=proprietary_product&a=get_product_list";
					}
					that.$common.requestData({
						url: url,
						data: search,
						...
						if (res.errcode == 0 || (res.data ? res.data.length > 0 : false)) {
							...
							res.data.forEach(function(itms, ind) {
								if (itm.commodity_type == 1) {
									...
									var store = {
										"id": itms.id,
										"pic":itms.img?itms.img[0]:"",
										"name":itms.name,
										"price":itms.price,
										"priceA": itms.plat_discount_type==1?util.toPrice(itms.price, true):((itms.price * itms.plat_discount)/10)>0.01?util.toPrice(((itms.price * itms.plat_discount)/10), true):'0',
										"priceB": itms.plat_discount_type==1?util.toPrice(itms.price, false):((itms.price * itms.plat_discount)/10)>0.01?util.toPrice(((itms.price * itms.plat_discount)/10), false):'.01',
										"sold":itms.sale_count,
										"distance":itms.distance,
										"discount":itms.plat_discount,
										"discountA":itms.plat_discount_type==1?util.toPrice(((itms.price / itms.package_price)*10), true):util.toPrice(((itms.price / itms.package_price)*itms.plat_discount), true),
										"discountB":itms.plat_discount_type==1?util.toPrice(((itms.price / itms.package_price)*10), false):util.toPrice(((itms.price / itms.package_price)*itms.plat_discount), false),
										"plat_discount_type":itms.plat_discount_type,
										"original_price":"",
										"group":"0",
										"store_name":itms.store_name,
										"store_id":itms.store_id,
										"give_integral":itms.give_integral,
										"integral_name":itms.integral_name
									};
								}
								if (itm.add_type == 1) {
									itm.dataset.push(store);
			...
			goUrl: function(e) {
				...
				this.$common.diyLinkJump('/shop_league/web/index.php?m=store&a=store_detail&id='+e.currentTarget.dataset.id)
			},
			goUrlshop: function(e) {
				const that = this;
				var itm = e.currentTarget.dataset.itm;
				if(that.datasetitms.commodity_type == 1){
					var url = "/shop_league/web/index.php?m=product&a=product_detail&id="+itm.id+"&store_id="+itm.store_id;
				}else if(that.datasetitms.commodity_type == 2){
					var url = "/shop_league/web/index.php?m=user_collage_product&a=collage_product_detail&collage_pid="+itm.id;
				}else if(that.datasetitms.commodity_type == 3){
					var url = "/shop_league/web/index.php?m=proprietary_product&a=product_detail&store_id="+itm.store_id+'&id=' + itm.id;
				}else if(that.datasetitms.commodity_type == 101){
					var url = "/shop_league/web/index.php?m=product&a=product_detail&id="+itm.id+"&store_id="+itm.store_id+"&is_plat_product=1";
				}
			...
			goShopDetail: function(e) {
				...
				that.$common.diyLinkJump("/shop_league/web/index.php?m=product&a=product_detail&store_id=" + id + "&id=" +pid)
			},
		}
	}
</script>
