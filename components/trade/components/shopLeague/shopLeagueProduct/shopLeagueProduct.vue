<template>
	<view :style="'padding:'+datas.content.padding_top+'px 0px '+datas.content.padding_bottom+'px;'">
		<view class="page-tab-main" v-if="datas.content.css_type == 1">
			...
			<view class="industry-main child" v-if="datas.content.show_industry_child">
				...
				<view class="industry-content">
					<view class="list flex-def flex-zBetween flex-cCenter"
						v-for="(itm,index) in datas.content.industry_list_child" :data-id="itm.id" :data-name="itm.name"
						@click="league_industryFun">
						<span
		...
		<view class="mescroll-main" :style="'padding: 0px '+datas.content.padding_horizontal+'px;'">
			<view class="mescroll-shop-list clearfloat">
				<view class="shop-list-new WaterfallsLeft">
					<view style="margin-bottom:10px" v-for="(itm,indexs) in datas.content.dataset_default"
						v-if="(datas.content.dataset.length == 0 || datas.content.add_type == 2)&&indexs % 2 == 0"
							...
							<view :class="'flex-def flex-cCenter '+(datas.content.integral?'flex-zBetween':'')">
								<view class="num flex-def flex-cCenter integral-box"
									...
									<view class="integral-box-left flex-def flex-cCenter flex-zCenter">
										{{itm.integral_name?itm.integral_name:'积分'}}
									</view>
									<view class="integral-box-right flex-def flex-cCenter flex-zCenter">
										{{itm.give_integral?itm.give_integral:'0.00'}}
									</view>
								</view>
					...
					<view style="margin-bottom:10px" v-for="(itm,index1) in datas.content.dataset"
						v-if="(datas.content.dataset.length > 0 && datas.content.add_type == 1)&&index1 % 2 == 0"
						@click="goUrl" :data-itm="itm">
						<view class="img-box" :style="'background-image: url('+itm.pic+');'">
							<view class="state flex-def flex-cStretch" v-if="datas.content.commodity_type == 2">
								<view class="type">拼团</view>
	...
	export default {
		...
		methods: {
				...
				if (that.shop_league_obj.industry_list && that.shop_league_obj.industry_list.length > 0) {
					...
					that.$common.requestData({
						...
						if (res.errcode == 0) {
							...
							that.datas.content.industry_list = that.datas.content.industry_list.concat(that
								.shop_league_type_list_format(res.data.list));
						}
					});
				}
			},
			...
			shop_league_get_list: function() {
				...
				var search = {
					store_type_id: that.datas.content.industry_type_1,
			...
			goUrl: function(e) {
				...
				if(that.datas.content.commodity_type == 1){
				    ...
				    var url = "/shop_league/web/index.php?m=user_collage_product&a=collage_product_detail&collage_product_detail&collage_pid="+itm.id;
				}
				// this.$common.diyLinkJump('/shop_league/web/index.php?m=product&a=product_detail&id=' + id);
				
				// if(that.datas.content.commodity_type == 1){
				//     var url = "/shop_league/pages/product/product_detail/product_detail?id="+itm.id+"&store_id="+itm.store_id;
				// }else if(that.datas.content.commodity_type == 2){
				//     var url = "/shop_league/pages/user_collage_product/collage_product_detail/collage_product_detail?collage_pid="+itm.id;
				// }
				console.log(url)
