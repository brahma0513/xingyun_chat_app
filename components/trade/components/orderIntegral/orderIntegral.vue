	...
	<view :style="'padding:'+datas.content.padding+'px 0;'">
		<view class="custom-order">
			<view class="type2" v-if="datas.content.css_type==2">
				<view hover-class="no-hover" url='/integral_shop/pages/order/orderList/orderList' open-type='navigate'>
			...
			<view class="type1" v-if="datas.content.css_type==1">
				<view class="title">
					...
					<view hover-class="no-hover"
						@click="$common.diyLinkJump('/integral_shop/web/index.php?m=order&a=order_list')">
						<p class="more">全部订单</p>
					</view>
				</view>
				<view class="order-list">
					<li v-for="(it,it_index1) in list" style="list-style: none;">
						<view class="nav-num" hover-class="no-hover"
							@click="$common.diyLinkJump('/integral_shop/web/index.php?m=order&a=order_list&status='+it_index1)">
							<view class="image">
								<image :src="it.icon" alt=""> </image>
							</view>
							<p>{{it.name}}</p>
							<block v-if='it.num>0'>
								<view class='reddot'>{{it.num}}</view>
							</block>
						</view>
					</li>
				</view>
