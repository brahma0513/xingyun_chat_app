	...
	<view :style="'padding:'+datas.content.padding+'px 0;'">
		<view class="custom-order">
			<view class="type2" v-if="datas.content.css_type==2">
				<view hover-class="no-hover" url='/longe_integral_shop/pages/order/orderList/orderList'
					open-type='navigate'>
			...
			<view class="type1" v-if="datas.content.css_type==1">
				<view class="title">
					...
					<view hover-class="no-hover"
						@click="$common.diyLinkJump('/longe_integral_shop/web/index.php?m=order&a=order_list')">
						<p class="more">全部订单</p>
					</view>
				</view>
				<view class="order-list" v-if="datas.content.css_type==1">
					<li v-for="(icon,icon_index) in list[0].icon_list" style="list-style: none;">
						<view class="nav-num" hover-class="no-hover"
							@click="$common.diyLinkJump('/longe_integral_shop/web/index.php?m=order&a=order_list&status='+icon_index)">
							<view class="image">
		...
		methods: {
			...
			get_list_num: function() {
				...
				that.$common.requestData({
					url: '/longe_integral_shop/web/index.php?m=order&a=get_order_num',
					data: {},
					method: 'POST',
					needToken: true
				}).then(res => {
