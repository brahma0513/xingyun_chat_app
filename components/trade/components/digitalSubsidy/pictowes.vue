<template>
	<view
		...
		<!-- swiper 部分：对应原小程序的swiper -->
		<swiper v-if="datas.content.shownum === 3 && datas.content.css_type === 3" class="swiper"
			:indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval" :duration="duration"
			:circular="true" :previous-margin="spacing + 'px'" :next-margin="spacing + 'px'" @change="changes"
			:style="{padding: datas.content.style_num === 1 ? '0 10px' : '0'}">
			<block v-for="(item, index) in datas.content.dataset" :key="index" v-if="index < datas.content.shownum">
				<swiper-item>
					<view class="swiper-item" :class="{active: index === current}">
						<!-- uni-app 中使用navigator，属性和小程序略有调整 -->
						<view :url="item.sel_link_type === 0 ? item.diy_link : item.link"
							:open-type="item.open_type ? item.open_type : 'navigate'"
							@click="go_urls(item.sel_link_type,item.sel_link_type == 0 ? item.diy_link : item.link)"
							>
		...
		<!-- 非swiper部分 -->
		<view v-else class="custom-case" :class="`case-${datas.content.shownum}`"
			:style="{padding: datas.content.style_num === 1 ? '0 10px' : '0'}">
			<view hover-class="no-hover" :class="[
	          datas.content.css_type === 1 ? 'saw' : 'tile',
	          ((datas.content.shownum === 3 || datas.content.shownum === 4) && (datas.content.spacing_bool === undefined || datas.content.spacing_bool === 2) && (datas.content.dividing_line_bool === undefined || datas.content.dividing_line_bool === 1)) ? 'bd' : '',
	        ...
	        ]" :style="{borderColor: datas.content.dividing_line_color !== undefined ? datas.content.dividing_line_color : '#333'}"
				v-for="(it, it_index) in datas.content.dataset" :key="it_index"
				:url="it.sel_link_type === 0 ? it.diy_link : it.link"
				:open-type="it.open_type ? it.open_type : 'navigate'" v-if="it_index < datas.content.shownum"
				@click="go_urls(it.sel_link_type,it.sel_link_type == 0 ? it.diy_link : it.link)"
				>
				<image class="case-box"
					:mode="datas.content.shownum === 2 || datas.content.css_type === 2 ? 'widthFix' : 'aspectFill'"
					:src="it.pic" lazy-load />
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			go_urls(type,url){
				...
				var link_type = type == 0? 'h5':'';
				this.$common.diyLinkJump(url,link_type,true,'','');
