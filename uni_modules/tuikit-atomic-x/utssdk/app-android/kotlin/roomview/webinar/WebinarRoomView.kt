...
import uts.sdk.modules.atomicx.kotlin.roomview.getScreenWidth
...
class WebinarRoomView @JvmOverloads constructor(
    ...
    private val logger = RoomViewLogger.getLogger("WebinarRoomView")
    ...
    /**
     * Drawable resources supplied by the host. Forwarded to the current
     * [VideoViewAdapter] when it is a [WebinarVideoViewAdapterImpl]. Custom
     * adapters provided via [setVideoViewAdapter] are unaffected.
     */
    ...
    /**
     * Inject (or replace) drawable resources used by the built-in widget views.
     * Safe to call before or after [setVideoViewAdapter]; whichever order the
     * host uses, the current adapter ends up with the latest icons.
     */
    fun setIcons(icons: RoomViewIcons) {
        this.icons = icons
        (videoViewAdapter as? WebinarVideoViewAdapterImpl)?.setIcons(icons)
    }

    fun setVideoViewAdapter(adapter: VideoViewAdapter?) {
        videoViewAdapter = adapter
        // Forward any previously-supplied icons to the new adapter so call order
        // (setIcons before setVideoViewAdapter, or vice-versa) does not matter.
        (adapter as? WebinarVideoViewAdapterImpl)?.setIcons(icons)
    }
