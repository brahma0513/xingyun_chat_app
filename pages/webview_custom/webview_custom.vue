<template>
    <view >
		<view class="container" style="background-color: #000000;">
			<web-view ref="webview" :src="weburl" @message="handlePostMessage"></web-view>
		</view>

		<view style="background-color: #000000;" v-if="show_pagecom">
...
<script>
    ...
    export default {
		...
		onLoad(e) {
			...
			if(that.vuex_apiUrl != 'https://yun.xingdian666.com'){
			...
			setTimeout(function() {
				...
				wv.addEventListener('titleUpdate', function(e){
					...
					// that.checkIsCanShare(currentUrl);
					if(e.title!='聊天室'&&e.title!='新的朋友'&&e.title!='添加好友'){
						return;
					}
					//云平台登陆链接
		...
		methods:{
			...
			handlePostMessage: function(data) {
				...
				if(event=='openMiniProgram'){
					...
					if(path != undefined && path!=""){						
						...
						//   appId: 'wx80005da9c3335750',
						//   path: '/public/pages/pay/payforapp/payforapp?order_id='+order_id,
						//   extraData: {
						//     'order_id': order_id
						//   },
						//   success(res) {
				...
				}else if(event=='getKeyBoardHeight'){
					//获取软键盘高度
					let listener = function(res){
						console.log("获取软键盘高度", res.height)
						var result = "{code:0,errMsg:'success',data:'"+res.height+"'}";
						that.wv.evalJS(`${return_data[0].callback}(${result})`);
			...
			check_is_pay_type(){
				...
				if(is_pay_page==true){
					...
					this.weburl = this.currentPageUrl+'&xdflag='+timestamp;
				}
			},
