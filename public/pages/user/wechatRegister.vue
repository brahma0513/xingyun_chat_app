	...
	<view>
		<view class="login">
			...
			<view class="login-top">
				...
				<view>{{$t('login.textContent_43')}}</view>
			</view>
			<view class="loginMain">
				<view>
					...
					<view>
						...
						<view class="form-row flex a-c" v-if="wechat_bind_phone == 0">
						...
						<block v-if="params.country_code=='+86'">
							<view class="flex a-c send-code-row">
								...
								<view v-if="wechat_bind_phone == 0" class="send-code-bnt" :class="['skin-color-'+theme,'skin-bd-'+theme]"
									@tap="vuex_uniappSet.is_tuxing_yzm>0? tuxingYz_fun('regist'):registGetCode()">
									<u-code ref="uCode" @change="codeChange" seconds="60"
										:startText="$t('login.textContent_10')"
										:changeText="'X'+$t('login.textContent_2')"></u-code>
									{{tips}}
								</view>
								<view v-else class="send-code-bnt" :class="['skin-color-'+theme,'skin-bd-'+theme]"
									@tap="vuex_uniappSet.is_tuxing_yzm>0? tuxingYz_fun('regist'):bindPhoneGetCode()()">
									<u-code ref="uCode" @change="codeChange" seconds="60"
										:startText="$t('login.textContent_10')"
										:changeText="'X'+$t('login.textContent_2')"></u-code>
									{{tips}}
						...
						<view class="form-row flex a-c" v-show="false">
				...
				<template v-if="mode=='accountRegister'">
					<view v-if="wechat_bind_phone == 0" :class="'registerBtn skin-bg-'+theme+''" style="margin-top: 60rpx;margin-bottom:32rpx;"
						@click="accountRegister()">
						{{$t('login.textContent_42')}}
					</view>
				</template>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			that.wechat_bind_phone = e.wechat_bind_phone??0;
			that.wechat_unionId = e.unionId??'';
		...
		methods: {
			wechat_register(){
				...
				params['unionId'] = that.wechat_unionId;
				that.$api.wechatRegister(params).then(res => {
					if (res.errcode == 0) {
						that.accountLogin();
					} else {
						uni.showToast({
			...
			bind_phone(){
				...
				var params = that.params;
				params['unionId'] = that.wechat_unionId;
				that.$api.wechatBindPhone(params).then(res => {
					console.log('wechatBindPhone',res);
					if (res.errcode == 0) {
			...
			bindPhoneGetCode() {
				...
				if (this.$refs.uCode.canGetCode) {
					...
					this.$api.getPhoneCode({
						bind_phone:that.params.phone,
						country_code:that.params.country_code
						}).then(res => {
						uni.hideLoading();
						if (res.errcode == 0) {
							// 这里此提示会被this.start()方法中的提示覆盖
